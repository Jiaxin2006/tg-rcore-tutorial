...
void cmap_to_fb(uint8_t *out, uint8_t *in, int in_pixels)
{
    ...
    for (i = 0; i < in_pixels; i++)
    {
        ...
        {
            // Assuming RGBA8888 (or XRGB with alpha field). I_SetPalette leaves
            // colors[i].a = 0; if we omit alpha here, VirtIO-GPU treats pixels as
            // fully transparent and the window stays black.
            pix = (c.r << s_Fb.red.offset) |
                  (c.g << s_Fb.green.offset) |
                  (c.b << s_Fb.blue.offset);
            if (s_Fb.transp.length > 0) {
                uint32_t amask =
                    (((uint32_t)1 << s_Fb.transp.length) - 1u) << s_Fb.transp.offset;
                pix |= amask;
            }
